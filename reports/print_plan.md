...
2. **Hidden-seam M4 insert coupon.** Before printing either revised shell, print `coupon_m4_seam_insert` in the same material, layer height, wall settings, and upright orientation as `lower_shell`. Its three vertical production-style bosses are Ø5.4, Ø5.6, and Ø5.8, identified by one, two, and three edge notches. Select the hole that installs the exact M4 insert concentrically without cracking, spinning, or pull-out; report the marker result before the shell STLs are released for printing.
...
9. **Router pad/fastener trial.** Print four 8 x 16 x 1.3 pad candidates and a representative low-head M3x6 tray joint. Verify the actual router has safe lands, nominal 0.4 compression is appropriate, all four contacts share load without rocking, the recess seats the selected diameter-6.2-or-smaller head without contact or blind-pilot bottoming, vents remain open, and repeated -Y insertion/+Y removal is smooth.
...
After the M4 seam coupon is reported and its selected pilot is regenerated, print `lower_shell` first and inspect the six insert bosses plus the new rear angle beam. Then print `upper_shell` and perform a dry six-screw seam assembly before installing the router tray or cap. Other already-passed or already-printed parts do not need reprinting unless their interface changed.
